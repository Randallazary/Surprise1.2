    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      {loading ? (
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-gray-600">Cargando políticas de privacidad...</p>
        </div>
      ) : politicas ? (
        <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden">
            ...
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">
                  {politicas.title || "Políticas de Privacidad"}
                </h1>
                <div className="flex items-center text-blue-100">
                  ...
                  <span className="text-sm sm:text-base">
                    Vigente desde: {politicas.effectiveDate ? new Date(politicas.effectiveDate).toLocaleDateString('es-ES', { 
                      year: 'numeric', 
                      month: 'long', 
                      day: 'numeric' 
                    }) : "No disponible"}
          ...
          <div className="px-6 py-8 sm:px-10 sm:py-12">
            <div className="prose max-w-none">
              {politicas.content ? (
                <div dangerouslySetInnerHTML={{ __html: politicas.content }} />
              ) : (
          ...
          <div className="bg-gray-50 px-6 py-4 sm:px-10 border-t border-gray-200">
            <div className="flex flex-col sm:flex-row justify-between items-center text-sm text-gray-500">
              <div className="mb-2 sm:mb-0">
                <span className="font-medium">Última actualización:</span> {politicas.updatedAt ? new Date(politicas.updatedAt).toLocaleDateString() : "No disponible"}
              </div>
              <div>
                <span className="font-medium">Creado el:</span> {politicas.createdAt ? new Date(politicas.createdAt).toLocaleDateString() : "No disponible"}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-md mx-auto bg-white p-8 rounded-xl shadow-md text-center">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">Error al cargar las políticas</h3>
          <p className="mt-2 text-gray-600">No se pudieron cargar las políticas de privacidad. Por favor, inténtalo de nuevo más tarde.</p>
          <button
